 ...
 * @LastEditors: 郭印林
 * @LastEditTime: 2022-09-20 16:06:49
...
其中，$p_\theta(\mathbf{x}|\mathbf{z})$ 是 decoder，$q_\phi(\mathbf{z}|\mathbf{x})$ 是 encoder，通过梯度下降进行优化。

DDPM 可以被看成是固定编码器的分层马尔可夫VAE。 forward 过程代表编码器，reverse 过程代表解码器，且 latent variable 和数据样本的维度一样。或者说，这里的 $q_\phi(\mathbf{z}|\mathbf{x})$ 类比于 Diffusion 中的 $p(\mathbf{x}_t|\mathbf{x}_{t-1})$，而 $p_\theta(\mathbf{x}|\mathbf{z})$ 类比于 Diffusion 中的 $q(\mathbf{x}_{t-1}|\mathbf{x}_{t})$
